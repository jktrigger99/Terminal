...
Или через цикл в одну строку:
`for i in {1..13}; do echo "5_line$i"; done > tF_5.txt`
...
`find . -iname "*tf*"`
Параметр `-i` - ignore, ищет в любом решистре
...
grep -w sec * (-w - whole, ищет целые слова)
...
Если во всех поддиректориях, то
grep -r second 
...
grep -sl second * (выводит только название/относительный путь) ???

 31. Найти все строки во всех файлах где нет комбинации “second”

grep -rv second (-v - reverse, ищет всё кроме искомой строки)
...
grep -rh "sec" | xargs -I{} echo {} > tf_6.txt (-h обрезает путь и имя файла)
...
echo Good job\!! (символы "!!" надо экранировать, так как это является командой)
